...
class GaussianProcess:
    ...
    def run(self, feat_train, feat_val, loc_train, loc_val, year_train, year_val,
            train_yield, model_weights, model_bias):

        # makes sure the features have an additional value for the bias term
        ...
        H_val = np.concatenate((feat_val, np.ones((feat_val.shape[0], 1))), axis=1)

        Y_train = np.expand_dims(train_yield, axis=1)

        n_train = feat_train.shape[0]
        n_val = feat_val.shape[0]

        locations = self._normalize(np.concatenate((loc_train, loc_val), axis=0))
        years = self._normalize(np.concatenate((year_train, year_val), axis=0))
